...
> medium-term after the user decided to ship Phase 12 (Mini Jobs queue +
> console) first and revisit whether a separate orchestrator is still
> warranted afterward. Phase 12's `Job` framework already absorbs most
> of the plumbing this document describes — typed job queue, single
> worker, audit JSONL, console surface, recipe registry, adapter pattern.
> An orchestrator built *on top of* Phase 12 may still make sense (the
> tier-separated trust model and Opus-review workflow are the parts least
> served by Phase 12 alone), but pre-building it before the meal-planner
> work ships would be wasted work. **Do not start implementation.
> Re-evaluate the design after Phase 12 + the meal-planner overhaul
> (Phase 13 architect + Phase 14+ build) ship.**
>
...
> daily Slack digest layer 2026-04-30) ✅, Phase 7 (NAS backup, shipped
> 2026-05-01) ✅, and Phase 12 (Mini Jobs queue + console, shipped
> 2026-05-01) ✅ are all complete. Still required before re-opening this
> design: Phase 12.5 (event-aggregator fetch+worker migration) and the
> meal-planner overhaul (Phase 13 architect + Phase 14+ build). Only
> after all of those have shipped should this design be re-opened for
> evaluation.
